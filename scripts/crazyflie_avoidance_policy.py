...
def main() -> None:
    ...
    parser.add_argument("--controller", choices=("policy", "reactive"), default="policy")
        ...
        try:
            ...
            with modules.sync_logger_cls(scf, build_log_configs(modules, config)) as logger:
                while time() < deadline:
                    ...
                    command = build_command(model, reading, args).clipped(
                        max_speed=min(args.max_speed_m_s, 0.35),
                    ...
                    rows.append({"host_time_s": time(), **latest, **command_row(command), "vz_m_s": vz_m_s})
        finally:
            if airborne:
                motion.stop()
                sleep(0.5)
...
def build_command(model: RangerAvoidancePolicy | None, reading, args):
    ...
    if model is None:
        raise SystemExit("--checkpoint is required when --controller policy")
    return command_from_model(model, reading)
