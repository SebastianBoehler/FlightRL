...
static PyObject *sixdof_step_env_context(PyObject *self, PyObject *args) {
    ...
    int reward_mode;
    float dt;
    if (!PyArg_ParseTuple(args, "OOOOOOOOOOOOOOOOiOf", &pos_obj, &vel_obj, &quat_obj, &rates_obj, &ranges_obj,
            &target_obj, &target_yaw_obj, &prev_obj, &step_count_obj, &actions_obj, &obs_obj, &rewards_obj,
    ...
    if (!pos || !vel || !quat || !rates || !ranges || !target || !target_yaw || !prev || !step_count || !actions || !obs || !rewards ||
    ...
    flightrl_sixdof_step_env_context_batch(
        PyArray_DATA(pos), PyArray_DATA(vel), PyArray_DATA(quat), PyArray_DATA(rates), PyArray_DATA(ranges),
        PyArray_DATA(target), PyArray_DATA(target_yaw), PyArray_DATA(prev), PyArray_DATA(step_count), PyArray_DATA(actions),
