...
def main() -> None:
    parser = argparse.ArgumentParser(description="Plan or run TorchScript desktop evidence generation for candidate-matrix checkpoints")
    ...
    parser.add_argument("--output-dir", default="artifacts/desktop")
    parser.add_argument(
        "--report",
        default="artifacts/replay/sixdof_desktop_evidence_manifest.json",
    )
    ...
    report = {
        ...
        "matrix": args.matrix,
        "records": records,
        "matrix_args": matrix_args(records),
        "safety": "Desktop CPU evidence only; not GAP8 deployment or live-flight authority.",
    }
    output = Path(args.report)
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(json.dumps(report, indent=2, sort_keys=True) + "\n")
    output.with_suffix(".md").write_text(render_markdown(report) + "\n")
    print(f"desktop_evidence={output}")
...
def evidence_record(record: dict, args: argparse.Namespace) -> dict:
    ...
    return {
        ...
        "model": str(model),
        "desktop_parity": str(parity),
        "desktop_latency": str(latency),
        "commands": [
...
def export_command(checkpoint: str, model: Path, parity: Path, samples: int) -> list[str]:
    return [
        sys.executable,
        str(ROOT / "scripts" / "export_sixdof_desktop_policy.py"),
        "--checkpoint",
        checkpoint,
        "--output",
        str(model),
...
def matrix_args(records: list[dict]) -> list[str]:
    ...
    for record in records:
        args.extend(
            [
                "--desktop-parity",
                f"{record['label']}={record['desktop_parity']}",
                "--desktop-latency",
                f"{record['label']}={record['desktop_latency']}",
            ]
        )
    return args
...
def render_markdown(report: dict) -> str:
    lines = [
        "# 6-DoF Desktop Evidence Manifest",
        "",
        "| label | tasks | status | parity | latency |",
        "| --- | --- | --- | --- | --- |",
    ]
    for record in report["records"]:
        lines.append(f"| {record['label']} | {', '.join(record['tasks'])} | {status(record)} | `{record['desktop_parity']}` | `{record['desktop_latency']}` |")
    lines.extend(["", report["safety"], "", "Append `matrix_args` from the JSON report to `build_sixdof_candidate_matrix.py`."])
    return "\n".join(lines)
