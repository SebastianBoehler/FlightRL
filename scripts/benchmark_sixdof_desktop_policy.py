...
from flightrl.desktop import sample_sixdof_observations
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Benchmark 6-DoF checkpoint and optional TorchScript desktop inference latency")
    parser.add_argument("--checkpoint", required=True)
    parser.add_argument("--torchscript", default=None)
    parser.add_argument("--output", default="artifacts/desktop/sixdof_latency.json")
    ...
    report = {
        ...
        "torchscript": (
            file_identity(args.torchscript) if args.torchscript else None
        ),
        ...
        "safety": "Desktop CPU latency only; it says nothing about GAP8 latency, memory fit, deployment readiness, or live-flight authority.",
    }
    if args.torchscript:
