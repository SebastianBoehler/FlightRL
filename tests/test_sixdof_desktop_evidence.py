...
SPEC = importlib.util.spec_from_file_location("build_sixdof_desktop_evidence", ROOT / "scripts" / "build_sixdof_desktop_evidence.py")
...
def test_matrix_args_pairs_parity_and_latency() -> None:
    ...
    assert set(record) >= {"desktop_parity", "desktop_latency"}
    assert EVIDENCE.matrix_args([record]) == [
        "--desktop-parity",
        "multi=artifacts/desktop/sixdof_multi.parity.json",
        "--desktop-latency",
        "multi=artifacts/desktop/sixdof_multi.latency.json",
    ]


def test_desktop_evidence_dry_run_writes_manifest(tmp_path: Path) -> None:
    matrix_path = tmp_path / "matrix.json"
    output = tmp_path / "desktop_manifest.json"
    matrix_path.write_text(json.dumps(matrix()))

    subprocess.run(
        [
            sys.executable,
            str(ROOT / "scripts" / "build_sixdof_desktop_evidence.py"),
            "--matrix",
            str(matrix_path),
    ...
    assert "--desktop-parity" in report["matrix_args"]
