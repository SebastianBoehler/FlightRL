...
Position/yaw reset profiles now step from `position_yaw_easy` to `position_yaw_medium`, `position_yaw_wide`, `position_yaw_hard`, then `broad`. The wide/hard profiles are relative-target bridges; they are intended for curriculum experiments before fully random room-level targets.
