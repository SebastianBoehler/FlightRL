...
def test_precontact_drift_brake_shapes_open_space_pitch_against_velocity() -> None:
    env = SixDofEnv(num_envs=1, seed=41, task="obstacle_avoidance", reset_profile="obstacle_hover_live")
    env.ranges_m[:, :4] = 1.2
...
def test_precontact_drift_brake_uses_body_frame_velocity() -> None:
    env = SixDofEnv(num_envs=1, seed=42, task="obstacle_avoidance", reset_profile="obstacle_hover_live")
    env.ranges_m[:, :4] = 1.2
    env.velocity[:] = np.asarray([[1.5, 0.0, 0.0]], dtype=np.float32)
...
def test_precontact_drift_brake_does_not_override_close_obstacle_targets() -> None:
    env = SixDofEnv(num_envs=1, seed=43, task="obstacle_avoidance", reset_profile="obstacle_hover_live")
    env.ranges_m[:, :4] = 0.35
    env.velocity[:] = np.asarray([[1.5, 0.0, 0.0]], dtype=np.float32)
...
def test_precontact_drift_brake_strength_scales_target_blend() -> None:
    env = SixDofEnv(num_envs=1, seed=44, task="obstacle_avoidance", reset_profile="obstacle_hover_live")
    env.ranges_m[:, :4] = 1.2
    env.velocity[:] = np.asarray([[1.5, 0.0, 0.0]], dtype=np.float32)
    target = np.zeros((1, 4), dtype=np.float32)
