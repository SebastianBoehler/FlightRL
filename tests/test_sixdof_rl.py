...
def test_ppo_update_runs_on_short_rollout() -> None:
    env = SixDofEnv(num_envs=4, seed=3, reset_profile="position_yaw_easy")
...
def test_collect_rollout_labels_teacher_on_recorded_state() -> None:
    env = SixDofEnv(num_envs=4, seed=5, reset_profile="position_yaw_easy")
...
def test_collect_rollout_teacher_residual_executes_teacher_and_trains_zero_residual() -> None:
    env = SixDofEnv(num_envs=4, seed=6, reset_profile="position_yaw_easy")
    expected = teacher_actions(env, task=env.task)
...
def test_collect_rollout_supports_progress_reward() -> None:
    env_progress = SixDofEnv(num_envs=4, seed=7, reset_profile="position_yaw_easy")
    env_clearance = SixDofEnv(num_envs=4, seed=7, reset_profile="position_yaw_easy")
    env_live = SixDofEnv(num_envs=4, seed=7, reset_profile="position_yaw_easy")
    env_base = SixDofEnv(num_envs=4, seed=7, reset_profile="position_yaw_easy")
    model = SixDofActorCritic(input_dim=28, hidden_size=16)
...
def test_yaw_clearance_reward_penalizes_true_angle_error() -> None:
    env = SixDofEnv(num_envs=2, seed=9, reset_profile="position_yaw_easy")
...
def test_circle_progress_reward_uses_orbit_error_not_center_error() -> None:
    env = SixDofEnv(num_envs=1, seed=10, task="circle", reset_profile="circle_recovery")
...
def test_collect_rollout_supports_history_observation_mode() -> None:
    env = SixDofEnv(num_envs=4, seed=11, reset_profile="position_yaw_easy")
...
def test_collect_rollout_supports_task_conditioned_observations() -> None:
    env = SixDofEnv(num_envs=6, seed=12, reset_profile="position_yaw_easy")
...
def test_collect_rollout_applies_initial_task_yaw_context() -> None:
    ...
    env = SixDofEnv(num_envs=2, seed=12, task="position_yaw")
